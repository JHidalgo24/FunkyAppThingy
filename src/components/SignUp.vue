...
export default {
    ...
    methods:{
      async registerUser(){
          await auth.createUserWithEmailAndPassword(this.user.email, this.user.password).then(c => {console.log(c)})
          ...
          await db.collection('Users').doc(this.user.uid).set({user:Object.assign({},this.user)})
      }
    }
}
</script>
